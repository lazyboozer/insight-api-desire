<h1 align="center">Insight-api-desire</h1>

<div align="center">
  <strong>A DESIRE blockchain REST and WebSocket API Service</strong>
</div>
<br />
<div align="center">
  <!-- Stability -->
  ...
  <!-- Build Status -->
  <a href="https://travis-ci.org/lazyboozer/insight-api-desire">
    <img src="https://img.shields.io/travis/lazyboozer/insight-api-desire/master.svg?style=flat-square" alt="Build Status" />
  </a>
  <!-- NPM version -->
  <a href="https://npmjs.org/package/insight-api-desire">
    <img src="https://img.shields.io/npm/v/insight-api-desire.svg?style=flat-square" alt="NPM version" />
  </a>
</div>


This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/lazyboozer/insight-ui-desire.
...
- [Resources](#resources)
- [License](https://github.com/lazyboozer/insight-api-desire/blob/master/LICENSE)

## Getting Started

```bashl
npm install -g bitcore-node-desire@latest
bitcore-node-desire create mynode
cd mynode
bitcore-node-desire install insight-api-desire
bitcore-node-desire start
```

The API endpoints will be available by default at: `http://localhost:3001/insight-api-desire/`

### Prerequisites

- [Bitcore Node DESIRE 3.x](https://github.com/lazyboozer/bitcore-node-desire)

**Note:** You can use an existing DESIRE data directory, however `txindex`, `addressindex`, `timestampindex` and `spentindex` needs to be set to true in `desire.conf`, as well as a few other additional fields.
...
### Block
```
  /insight-api-desire/block/[:hash]
  /insight-api-desire/block/0000000006e7b38e8ab2d351239019c01de9a148b5baef58cfe52dfd9917cedc
```

### Block Index
Get block hash by height
```
  /insight-api-desire/block-index/[:height]
  /insight-api-desire/block-index/0
```
This would return:
```
{
...
### Raw Block
```
  /insight-api-desire/rawblock/[:blockHash]
```
...
```
  /insight-api-desire/blocks?limit=3&blockDate=2017-04-22
```
...
### Transaction
```
  /insight-api-desire/tx/[:txid]
  /insight-api-desire/tx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
  /insight-api-desire/rawtx/[:rawid]
  /insight-api-desire/rawtx/ebdca263fe1c75c8609ce8fe3d82a320a0b3ca840f4df995883f5dab1b9ff8d9
```

### Address
```
  /insight-api-desire/addr/[:addr][?noTxList=1][&from=&to=]
  /insight-api-desire/addr/ybi3gej7Ea1MysEYLR7UMs3rMuLJH5aVsW?noTxList=1
  /insight-api-desire/addr/yPv7h2i8v3dJjfSH4L3x91JSJszjdbsJJA?from=1000&to=2000
```

### Address Properties
```
  /insight-api-desire/addr/[:addr]/balance
  /insight-api-desire/addr/[:addr]/totalReceived
  /insight-api-desire/addr/[:addr]/totalSent
  /insight-api-desire/addr/[:addr]/unconfirmedBalance
```
The response contains the value in Satoshis.

### Unspent Outputs
```
  /insight-api-desire/addr/[:addr]/utxo
```
Sample return:
```
[
...
GET method:
```
  /insight-api-desire/addrs/[:addrs]/utxo
  /insight-api-desire/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/utxo
```

POST method:
```
  /insight-api-desire/addrs/utxo
```
...
```
  /insight-api-desire/txs/?block=HASH
  /insight-api-desire/txs/?block=000000000814dd7cf470bd835334ea6624ebf0291ea857a5ab37c65592726375
```
### Transactions by Address
```
  /insight-api-desire/txs/?address=ADDR
  /insight-api-desire/txs/?address=yWFfdp9nLUjy1kJczFhRuBMUjtTkTTiyMv
```

### Transactions for Multiple Addresses
GET method:
```
  /insight-api-desire/addrs/[:addrs]/txs[?from=&to=]
  /insight-api-desire/addrs/ygwNQgE5f15Ygopbs2KPRYMS4TcffqBpsz,ygw5yCtVkx3hREke4L8qDqQtnNoAiPKTSx/txs?from=0&to=20
```

POST method:
```
  /insight-api-desire/addrs/txs
```
...
GET method:
```
  /insight-api-desire/gobject/list/proposal
```
...
GET method:
```
  /insight-api-desire/gobject/get/[:hash]
  /insight-api-desire/gobject/get/b6af3e70c686f660541a77bc035df2e5e46841020699ce3ec8fad786f7d1aa35
```
...
### Masternodes List
```
  /insight-api-desire/masternodes/list
```
### Validate Masternode
```
  /insight-api-desire/masternodes/validate/[:payee]
  /insight-api-desire/masternodes/validate/yRuALkPpeYpTgxdNn2L5YgGktASJYDYPAo
```
...
```
  /insight-api-desire/sync
```

### Live Network P2P Data Sync Status
```
  /insight-api-desire/peer
```

### Status of the Bitcoin Network
```
  /insight-api-desire/status?q=xxx
```
...
### Utility Methods
```
  /insight-api-desire/utils/estimatefee[?nbBlocks=2]
```
...
- (Medium)[How to setup a DESIRE Instant-Send Transaction using Insight API?????????The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)
